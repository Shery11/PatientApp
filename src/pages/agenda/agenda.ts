...
export class AgendaPage {
  ...
  constructor(public storage: Storage, public navCtrl: NavController, public navParams: NavParams,
    public loadingCtrl : LoadingController,public alertCtrl: AlertController) {
      ...
      this.storage.get('userKey').then(key=>{
        ...
        firebase.database().ref('userProfile/'+key+'/agendas').on('value',snapShot=>{
          console.log(snapShot.val());
          this.loading.dismiss();
  
          this.AgendasArr = snapshotToArray(snapShot);
         this.AgendasArr = this.AgendasArr.reverse();
        })
  ...
  edit(id){
    ...
    this.navCtrl.push('AddagendaPage', {id : id});
